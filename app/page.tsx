...
export default function Home() {
  ...
  return (
    <div className="noise" style={{ background: '#09090b', minHeight: '100vh', width: '100%', overflowX: 'hidden' }}>
      ...
      {/* ===== HERO ===== */}
      <section className="grid-bg" style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden', width: '100%' }}>
        {/* Glow orbs */}
        ...
        {/* Nav */}
        <nav style={{ position: 'relative', zIndex: 20, width: '100%' }}>
          <div style={{ ...containerStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '24px', paddingBottom: '24px' }}>
            {!isMobile && (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '20px' : '40px', marginLeft: 'auto' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ width: '28px', height: '28px', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#2563EB' }}>
              ...
              {!isMobile && (
                <div style={{ display: 'flex', gap: '36px' }}>
                  {NAV_LINKS.map((link) => (
                    <button key={link.id} onClick={() => scrollTo(link.id)}
                      style={{ fontSize: '13px', color: 'rgba(244,244,245,0.5)', letterSpacing: '0.03em', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)', transition: 'color 0.3s' }}
              ...
              {isMobile && (
                <button style={{ display: 'flex', flexDirection: 'column', gap: '5px', padding: '8px', background: 'none', border: 'none', cursor: 'pointer' }}
                  onClick={() => setMobileMenu(!mobileMenu)} aria-label="Toggle menu"
                >
                  <span style={{ display: 'block', width: '20px', height: '1.5px', background: 'white', transition: 'all 0.3s', transform: mobileMenu ? 'rotate(45deg) translateY(3.25px)' : 'none' }} />
                  <span style={{ display: 'block', width: '20px', height: '1.5px', background: 'white', transition: 'all 0.3s', transform: mobileMenu ? 'rotate(-45deg) translateY(-3.25px)' : 'none' }} />
                </button>
              )}
            </div>
        ...
        <div style={{ ...containerStyle, position: 'relative', zIndex: 10, paddingBottom: isMobile ? '40px' : '56px' }}>
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '40px' : '48px', minHeight: isMobile ? 'auto' : 'calc(100vh - 130px)' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0, gap: isMobile ? '40px' : 0 }}>
              ...
              {/* Headline */}
              <div style={{ maxWidth: isMobile ? '100%' : '780px' }} key={fadeKey}>
                <h1 className="animate-fade-up" style={{
                  fontFamily: 'var(--font-display)', fontWeight: 400,
                  fontSize: 'clamp(2rem, 4.2vw, 4.8rem)', lineHeight: 1.06,
                  letterSpacing: '-0.03em', color: '#f4f4f5', marginBottom: '28px',
                }}>
                ...
                <p className="animate-fade-up delay-400" style={{ fontSize: isMobile ? '14px' : '15px', maxWidth: '520px', lineHeight: 1.7, color: 'rgba(244,244,245,0.5)' }}>
                  {slide.desc}
                </p>
              </div>
      ...
      <section id="platform" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ ...containerStyle, paddingTop: isMobile ? '64px' : '120px', paddingBottom: isMobile ? '64px' : '120px' }}>
          <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 600, marginBottom: '20px', color: '#2563EB' }}>What we&apos;re building</p>
          ...
          <p style={{ fontSize: isMobile ? '14px' : '15px', lineHeight: 1.75, maxWidth: '620px', color: 'rgba(244,244,245,0.5)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(2, 1fr)', gap: '16px', marginTop: isMobile ? '40px' : '56px' }}>
            {PLATFORM_CARDS.map((card, i) => (
              <div key={i}
                style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: isMobile ? '24px' : '28px', transition: 'all 0.3s' }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(37,99,235,0.3)'; e.currentTarget.style.background = 'rgba(37,99,235,0.03)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'; e.currentTarget.style.background = 'rgba(255,255,255,0.02)'; }}
              >
                <div style={{ color: '#2563EB', marginBottom: '14px' }}>{card.icon}</div>
                <h3 style={{ fontSize: '15px', fontWeight: 500, marginBottom: '8px', color: '#f4f4f5', fontFamily: 'var(--font-display)' }}>{card.title}</h3>
                <p style={{ fontSize: '13px', lineHeight: 1.65, color: 'rgba(244,244,245,0.45)' }}>{card.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="research" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ ...containerStyle, paddingTop: isMobile ? '64px' : '120px', paddingBottom: isMobile ? '64px' : '120px' }}>
          <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 600, marginBottom: '20px', color: '#2563EB' }}>Why this matters</p>
          ...
          <p style={{ fontSize: isMobile ? '14px' : '15px', lineHeight: 1.75, maxWidth: '620px', marginTop: '16px', color: 'rgba(244,244,245,0.5)' }}>
          ...
          <p style={{ fontSize: isMobile ? '14px' : '15px', marginTop: '24px', fontWeight: 500, color: '#2563EB' }}>
            Define AI exists to make sure the first path is the easy path.
          </p>

          {/* Principles */}
          <div style={{ marginTop: isMobile ? '64px' : '80px' }}>
            <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 600, marginBottom: '24px', color: '#2563EB' }}>Principles</p>
            <div>
              {PRINCIPLES.map((p, i) => (
                <div key={i}
                  style={{
                ...
                >
                  <span style={{ fontSize: isMobile ? '13px' : '14px', fontWeight: 500, color: '#f4f4f5', fontFamily: 'var(--font-display)' }}>{p.label}</span>
                  <span style={{ fontSize: isMobile ? '13px' : '14px', lineHeight: 1.7, color: 'rgba(244,244,245,0.45)' }}>{p.text}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Roadmap */}
          <div style={{ marginTop: isMobile ? '64px' : '80px' }}>
            <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 600, marginBottom: '24px', color: '#2563EB' }}>Roadmap</p>
            <div>
              {ROADMAP.map((r, i) => (
                <div key={i}
                  style={{
                    ...
                    padding: '20px 0',
                    borderBottom: i < ROADMAP.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none',
                  }}
                >
                  <span style={{ fontSize: '12px', fontWeight: 600, flexShrink: 0, color: '#2563EB', minWidth: '56px', letterSpacing: '0.04em' }}>{r.phase}</span>
                  <span style={{ fontSize: isMobile ? '14px' : '15px', color: 'rgba(244,244,245,0.65)' }}>{r.text}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="industries" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ ...containerStyle, paddingTop: isMobile ? '64px' : '120px', paddingBottom: isMobile ? '64px' : '120px' }}>
          <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 600, marginBottom: '20px', color: '#2563EB' }}>Who we work with</p>
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: 'clamp(1.5rem, 3vw, 2.6rem)', lineHeight: 1.2, letterSpacing: '-0.025em', color: '#f4f4f5', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginTop: '32px' }}>
            {INDUSTRIES.map((ind) => (
              <span key={ind}
                style={{ fontSize: '13px', padding: '8px 16px', borderRadius: '999px', transition: 'all 0.3s', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(244,244,245,0.55)', cursor: 'default' }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(37,99,235,0.4)'; e.currentTarget.style.color = '#ffffff'; }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'; e.currentTarget.style.color = 'rgba(244,244,245,0.55)'; }}
              >{ind}</span>
            ))}
          </div>
        </div>
      ...
      <section id="about">
        <div style={{ ...containerStyle, paddingTop: isMobile ? '64px' : '120px', paddingBottom: isMobile ? '64px' : '120px' }}>
          <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 600, marginBottom: '20px', color: '#2563EB' }}>About</p>
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: 'clamp(1.5rem, 3vw, 2.6rem)', lineHeight: 1.2, letterSpacing: '-0.025em', color: '#f4f4f5', marginBottom: '20px' }}>
          ...
          <div style={{ marginTop: isMobile ? '40px' : '56px', padding: isMobile ? '24px' : '48px', borderRadius: '16px', background: 'rgba(37,99,235,0.04)', border: '1px solid rgba(37,99,235,0.12)' }}>
      ...
      <footer style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ ...containerStyle, paddingTop: isMobile ? '56px' : '96px', paddingBottom: '40px' }}>

          {/* Footer grid */}
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: isMobile ? '32px' : '48px', marginBottom: isMobile ? '48px' : '64px' }}>

            {/* Brand */}
            <div style={{ gridColumn: isMobile ? 'span 2' : 'auto' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', cursor: 'pointer' }} onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                <div style={{ width: '28px', height: '28px', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#2563EB' }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M12 2L2 7l10 5 10-5-10-5z" />
                    <path d="M2 17l10 5 10-5" />
              ...
              <p style={{ fontSize: '13px', lineHeight: 1.7, maxWidth: '280px', color: 'rgba(244,244,245,0.35)' }}>
            ...
            {/* Platform */}
            <div>
              <h4 style={{ fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: '20px', color: 'rgba(244,244,245,0.5)' }}>Platform</h4>
              {['AI Protection', 'Governance', 'Agent Security', 'Alignment', 'Documentation'].map((item) => (
                <a key={item} href="#platform" style={{ display: 'block', fontSize: '13px', marginBottom: '12px', color: 'rgba(244,244,245,0.35)', textDecoration: 'none', transition: 'color 0.2s' }}
                  onMouseEnter={(e) => (e.currentTarget.style.color = '#ffffff')}
                  onMouseLeave={(e) => (e.currentTarget.style.color = 'rgba(244,244,245,0.35)')}
                >{item}</a>
              ))}
            </div>
            ...
            {/* Company */}
            <div>
              <h4 style={{ fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: '20px', color: 'rgba(244,244,245,0.5)' }}>Company</h4>
              {['About', 'Research', 'Careers', 'Blog', 'Press'].map((item) => (
                <a key={item} href={`#${item.toLowerCase()}`} style={{ display: 'block', fontSize: '13px', marginBottom: '12px', color: 'rgba(244,244,245,0.35)', textDecoration: 'none', transition: 'color 0.2s' }}
                  onMouseEnter={(e) => (e.currentTarget.style.color = '#ffffff')}
                  onMouseLeave={(e) => (e.currentTarget.style.color = 'rgba(244,244,245,0.35)')}
                >{item}</a>
              ))}
            ...
            {/* Legal */}
            <div>
              <h4 style={{ fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: '20px', color: 'rgba(244,244,245,0.5)' }}>Legal</h4>
              {['Privacy Policy', 'Terms of Service', 'Cookie Policy', 'Security'].map((item) => (
                <a key={item} href="#" style={{ display: 'block', fontSize: '13px', marginBottom: '12px', color: 'rgba(244,244,245,0.35)', textDecoration: 'none', transition: 'color 0.2s' }}
                  onMouseEnter={(e) => (e.currentTarget.style.color = '#ffffff')}
                  onMouseLeave={(e) => (e.currentTarget.style.color = 'rgba(244,244,245,0.35)')}
                >{item}</a>
              ))}
          ...
          {/* Bottom bar */}
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: 'center', gap: '16px', paddingTop: '32px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <span style={{ fontSize: '12px', color: 'rgba(244,244,245,0.25)' }}>© 2025 Define AI. All rights reserved.</span>

            <div style={{ display: 'flex', gap: '20px' }}>
              <a href="#" style={{ color: 'rgba(244,244,245,0.3)', transition: 'color 0.2s' }} onMouseEnter={(e) => (e.currentTarget.style.color = '#ffffff')} onMouseLeave={(e) => (e.currentTarget.style.color = 'rgba(244,244,245,0.3)')}>
            ...
            <span style={{ fontSize: '12px', fontStyle: 'italic', color: 'rgba(244,244,245,0.25)' }}>Intelligence. Governed.</span>
          </div>
        </div>
      </footer>
    </div>
